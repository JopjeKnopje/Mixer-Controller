...
namespace Mixer_Controller
{
    ...
    // TODO Split the api interaction and pid fetching logic into 2 serperate classes

    /// <summary>
    /// used to talk with the windows audio mixer.
    /// thank you random stackoverflow user :) 
    /// </summary>
    public class MixerController
    {
        // contains for each app a list with all the processes that have a audioObject 
        // kinda like a Java HashMap
        private static IDictionary<string, List<Process>> appObjects = new Dictionary<string, List<Process>>();
        // timer used to trigger the audio object scan
        private static Timer timer = new Timer("UpdateAudioObjectList", true, false);

        // used to trigger the timer for the first time
        private static bool initUpdateAudioObjectList = true;


        // the interval the processes will be scanned at
        private const uint UPDATE_INTERVAL = 5000;



        /// <summary>
        /// TODO Clean up
        /// TODO Make another class to handle the logic that talks with this Windows API stuff
        /// </summary>
        /// <param name="app"></param>
        /// <param name="val"></param>
        //public static void setProgramVolume(string app, uint val)
        public static void setProgramVolume(string[] apps, uint val)
        {
            /// TODO for each app have a serperate timer;
            /// for example when the did chrome and its waiting 5 seconds
            /// it wont update discord.
            /// but if we give each app its own timer you can
            if (timer.delta() > UPDATE_INTERVAL || initUpdateAudioObjectList)
            {

                // TODO on init updateAudioObjectList() for all apps not just for the ones that are currently changing volume
                foreach (string app in apps)
                {
                    initUpdateAudioObjectList = false;
                    /// the SetApplicationVolume() method checks if the process even has a volume
                    /// Because when we try to set the volume of for example Chrome (which has 35 tabs) 
                    /// its very slow because it needs to loop through the processes[] to check if it has an audioObject every time!
                    /// thats why we call the updateAudioObjectList() method once every UPDATE_INTERVAL.
                    /// that way we have a list of apps of which we are sure have an audioObject.
                    /// this saves the SetApplicationVolume() a bunch of scanning, thus its way faster.

                    updateAudioObjectList(app);
                    timer.start();
                    Trace.WriteLine("updateAudioObjectList for " + app);
                }
            ...
            foreach (string app in apps)
            {
                // will give an error if we don't first call updateAudioObjectList()
                Process[] processes = appObjects[app].ToArray();

                foreach (Process p in processes)
                {
                    //SetApplicationVolume(p.Id, (float) val);
                    SetApplicationVolume(p.Id, val);
                }
            }
        ...
        /// <summary>
        /// runs updateAudioObjectList for all the possible apps
        /// </summary>
        ...
        private static void updateAudioObjectList(string app)
        {
            ...
            foreach (Process p in processes)
            {
                if (hasVolumeObject(p.Id))
                {
                    //Trace.WriteLine(p.Id + " has an audioObject");
                    Trace.WriteLine(p.ProcessName + " " + p.Id + " has an audioObject");
        ...
        /// <summary>
        /// checks if process has a volume object
        /// </summary>
        /// <param name="pid"></param>
        /// <returns></returns>
        public static bool hasVolumeObject(int pid)
        {
